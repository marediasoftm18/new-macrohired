  ...
  return (
    <section className="w-full bg-white py-14 md:py-24 px-4 sm:px-6 md:px-12 lg:px-16 font-sans">
      <div className="max-w-7xl mx-auto flex flex-col items-center gap-10 md:gap-12">
        ...
        {/* Filters Tabs */}
        <div className="flex flex-wrap justify-center gap-2 md:gap-3 my-2 sm:my-4">
          {categories.map((category) => (
            <button
              key={category}
              onClick={() => setActiveCategory(category)}
              className={`font-dm-sans px-4 sm:px-6 py-2 rounded-full font-normal text-[15px] sm:text-[18px] leading-7 transition-all duration-300 ${
            ...
            <AnimatePresence mode="popLayout">
              {filteredProjects.map((project) => (
                ...
                >
                  ...
                  <div className="bg-[#F6F5F2] rounded-b-3xl p-5 sm:p-7.5 flex justify-between items-center border-t border-gray-100/50 shadow-sm h-auto sm:h-25">
                    <h3
                      className="font-manrope font-semibold text-[22px] sm:text-[28px] lg:text-[32px] leading-tight sm:leading-10 text-[#044647]"
                    >
                      {project.title}
                    </h3>
                    <span
                      className="font-dm-sans font-semibold text-[12px] sm:text-[14px] leading-tight text-[#051B05] bg-white border border-[#051B05]/10 rounded-full px-3.5 sm:px-4 py-2 sm:py-2.5 shadow-sm shrink-0"
                    >
                      {project.tag}
                    </span>
                  </div>
