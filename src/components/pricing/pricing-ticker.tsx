...
export default function PricingTicker() {
  return (
    <section className="w-full py-4 sm:py-5 bg-[#C6D936] overflow-hidden flex items-center shadow-xs font-sans">
      <style>{`
        ...
        {[...Array(4)].map((_, setIdx) => (
          <div key={setIdx} className="flex gap-8 sm:gap-12 items-center shrink-0">
            {marqueeWords.map((word, wordIdx) => (
              <div
                key={`${setIdx}-${wordIdx}`}
                className="font-manrope flex items-center gap-3 text-[#051B05] font-semibold text-[18px] sm:text-[22px] md:text-[24px] lg:text-[28px] leading-9 select-none"
              >
                <img
                  src="/media assets/home/icon-24.svg"
                  alt="icon"
                  className="w-5 h-5 sm:w-6 sm:h-6 shrink-0 brightness-0"
                />
                <span>{word}</span>
              </div>
            ))}
